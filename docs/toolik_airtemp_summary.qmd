...
```{r}
# skim is a summary stats package
# do u need this line to run dim and names?
skimr::skim(toolik_chlorophyll)
```

```{r}
# clean_names() makes names easier to read
# in this case it just removes capital letters
toolik_chlorophyll <- toolik_chlorophyll %>% clean_names()
```


```{r}
# summry stats functions
dim(toolik_chlorophyll)
names(toolik_chlorophyll)
```

```{r}
# we are replacing blank values (in this case ".") with NAs 
# dollar sign operator grabs a column in a table
...
```{r}
#as.numeric turns a character into a numeric value, including NAs
...
toolik_chlorophyll_plot <- ggplot(data = toolik_chlorophyll, 
  ...
  scale_color_gradient(low = "blue", high = "orange") +
  theme_minimal() + labs(title = "faskjfawnkfjaf", x = "depth", y= "chloro conc (mm/L)")
```

```{r}
# safe the graph you just made
